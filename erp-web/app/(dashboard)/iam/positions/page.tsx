...
import type { Position, PageParams } from '@/types';
...
export default function PositionsPage() {
  ...
  const { data, mutate } = usePageApi<Position>('/iam/api/in/v1/positions', params);

  const handleCreate = () => { setEditing(null); form.resetFields(); setModalOpen(true); };
  const handleEdit = (record: Position) => { setEditing(record); form.setFieldsValue(record); setModalOpen(true); };

  const handleSubmit = async () => {
    try {
      const values = await form.validateFields();
      if (editing) { await iamApi.updatePosition(editing.positionId, values); }
      else { await iamApi.createPosition(values); }
  ...
  const columns = [
    ...
    { title: '操作', key: 'action', render: (_: unknown, record: Position) => <Button type="link" icon={<EditOutlined />} onClick={() => handleEdit(record)}>编辑</Button> },
  ];
  ...
  return (
    <div>
      ...
      <Card style={{ borderRadius: 8 }}>
        <Table rowKey="positionId" columns={columns} dataSource={data?.list || []} size="middle"
      ...
      <Modal title={editing ? '编辑岗位' : '新建岗位'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={480}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="level" label="岗位级别" rules={[{ required: true }]}><InputNumber min={1} max={10} style={{ width: '100%' }} /></Form.Item>
        </Form>
      </Modal>
    </div>
  );
