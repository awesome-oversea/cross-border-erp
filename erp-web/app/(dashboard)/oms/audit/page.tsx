...
const statusMap: Record<string, { color: string; text: string }> = {
  PENDING: { color: 'orange', text: '待审核' }, CONFIRMED: { color: 'blue', text: '已确认' }, REJECTED: { color: 'red', text: '已驳回' },
};

export default function OrderAuditPage() {
  const [params, setParams] = useState<PageParams & Record<string, unknown>>({ page: 1, size: 20, status: 'PENDING' });
  const [auditModalOpen, setAuditModalOpen] = useState(false);
  const [currentOrder, setCurrentOrder] = useState<Order | null>(null);
  const [form] = Form.useForm();
  const { data, mutate } = usePageApi<Order>('/oms/api/in/v1/orders', params);

  const handleAudit = (order: Order) => { setCurrentOrder(order); form.resetFields(); setAuditModalOpen(true); };
  ...
  const columns = [
    { title: '订单号', dataIndex: 'orderNo', key: 'orderNo', render: (v: string) => <a href={`/oms/orders?orderNo=${v}`}>{v}</a> },
    { title: '平台', dataIndex: 'platform', key: 'platform', render: (v: string) => <Tag>{v}</Tag> },
    { title: '客户', dataIndex: 'customerName', key: 'customerName', render: (v: string) => <a href={`/crm/customers?name=${v}`}>{v}</a> },
    ...
    { title: '操作', key: 'action', render: (_: unknown, record: Order) => (
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <Badge count={data?.total || 0} overflowCount={99}><Tag color="orange">待审核订单</Tag></Badge>
      </div>
      <Card style={{ borderRadius: 8 }}>
        <Table rowKey="orderId" columns={columns} dataSource={data?.list || []} size="middle"
      ...
      <Modal title={`审核订单 - ${currentOrder?.orderNo || ''}`} open={auditModalOpen} onOk={submitAudit} onCancel={() => setAuditModalOpen(false)} width={480}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="reason" label="审核意见"><Input.TextArea rows={3} placeholder="请输入审核意见" /></Form.Item>
        </Form>
      </Modal>
    </div>
  );
