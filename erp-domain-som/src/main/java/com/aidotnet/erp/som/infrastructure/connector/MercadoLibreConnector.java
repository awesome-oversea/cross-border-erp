...
@Component
public class MercadoLibreConnector implements PlatformConnector {

    private static final Logger log = LoggerFactory.getLogger(MercadoLibreConnector.class);
    private static final String PLATFORM_CODE = "MERCADO_LIBRE";
    private static final String PLATFORM_NAME = "Mercado Libre";
    ...
    public String authorize(String storeId, Map<String, String> authParams) {
        ...
        if (code == null || code.isBlank()) {
            throw new BizException("ML_AUTH_FAILED", "authorization_code不能为空");
        }
        log.info("Mercado Libre authorization initiated for store={}", storeId);
        return "ml_token_" + storeId;
    }
    ...
    public boolean refreshToken(String storeId, String refreshToken) {
        log.info("Refreshing Mercado Libre token for store={}", storeId);
        return refreshToken != null && !refreshToken.isBlank();
    }

    @Override
    public Map<String, Object> fetchOrders(String storeId, String accessToken, Map<String, String> params) {
        log.info("Fetching Mercado Libre orders for store={}", storeId);
    ...
    public Map<String, Object> fetchListings(String storeId, String accessToken, Map<String, String> params) {
        log.info("Fetching Mercado Libre listings for store={}", storeId);
        requireValidToken(storeId, accessToken);
        Map<String, Object> result = new HashMap<>();
        result.put("platform", PLATFORM_CODE);
        result.put("storeId", storeId);
    ...
    public boolean updateListing(String storeId, String accessToken, String listingId, Map<String, Object> data) {
        log.info("Updating Mercado Libre listing={} for store={}", listingId, storeId);
        requireValidToken(storeId, accessToken);
        return true;
    }
    ...
    public boolean updateInventory(String storeId, String accessToken, String sku, int quantity) {
        log.info("Updating Mercado Libre inventory for sku={} store={}", sku, storeId);
        requireValidToken(storeId, accessToken);
        return true;
    }
    ...
    public boolean markShipped(String storeId, String accessToken, String orderId, Map<String, String> shipmentInfo) {
        log.info("Shipping Mercado Libre order={} for store={}", orderId, storeId);
        requireValidToken(storeId, accessToken);
        return true;
    }
    ...
    public Map<String, Object> fetchReviews(String storeId, String accessToken, Map<String, String> params) {
        log.info("Fetching Mercado Libre reviews for store={}", storeId);
        requireValidToken(storeId, accessToken);
        Map<String, Object> result = new HashMap<>();
        result.put("platform", PLATFORM_CODE);
        result.put("storeId", storeId);
    ...
    private void requireValidToken(String storeId, String accessToken) {
        if (!validateToken(storeId, accessToken)) {
            throw new BizException("ML_TOKEN_INVALID", "Mercado Libre token无效，请重新授权");
        }
    }
}
