...
@Component
public class AvalaraTaxProvider implements TaxServiceProvider {

    private static final Logger log = LoggerFactory.getLogger(AvalaraTaxProvider.class);
    private static final String PROVIDER_CODE = "AVALARA";
    private static final String PROVIDER_NAME = "Avalara";
    private static final String BASE_URL = "https://rest.avatax.com/api/v2";
    ...
    public TaxCalculationResult calculateTax(String tenantId, String countryCode, String regionCode,
        ...
        log.info("Avalara calculating tax for tenant={} country={} region={} amount={}", tenantId, countryCode, regionCode, taxableAmount);
        try {
            BigDecimal taxRate = fetchTaxRate(countryCode, regionCode, params.getOrDefault("tax_code", ""));
            ...
            return new TaxCalculationResult(UUID.randomUUID().toString(), tenantId, countryCode, regionCode,
                    sellerSku, orderId, taxableAmount, taxRate, taxAmount, "VAT/GST", PROVIDER_CODE, Instant.now());
        } catch (Exception e) {
            log.error("Avalara tax calculation failed: {}", e.getMessage());
            throw new BizException("TAX_CALCULATION_FAILED", "Avalara税务计算失败: " + e.getMessage());
        }
    }

    @Override
    public VatValidationResult validateVat(String vatNumber, String countryCode) {
        log.info("Avalara validating VAT number={} country={}", vatNumber, countryCode);
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(BASE_URL + "/addresses/resolve?vat=" + vatNumber))
            ...
            log.error("Avalara VAT validation failed: {}", e.getMessage());
    ...
    private BigDecimal fetchTaxRate(String countryCode, String regionCode, String taxCode) {
        return switch (countryCode.toUpperCase()) {
            case "DE", "FR", "IT", "ES", "NL" -> new BigDecimal("0.19");
            case "GB" -> new BigDecimal("0.20");
            case "US" -> new BigDecimal("0.00");
